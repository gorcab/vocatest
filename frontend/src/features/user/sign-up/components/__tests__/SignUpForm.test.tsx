...
import {
  alreadyRegisteredEmailForAuthCodeResponse,
  failedSendingAuthCodeResponse,
  successSendingAuthCodeResponse,
  timedOutAuthCodeResponse,
  wrongAuthCodeForSignUpResponse,
...
describe("SignUpForm", () => {
  ...
  describe("인증 번호 필드를 올바르게 입력하지 않은 상태", () => {
    ...
    it('인증 번호를 요청한 뒤 시간 제한 내에 회원가입 버튼을 누르지 않으면 "인증 번호를 다시 요청해주세요." 메시지를 보여준다.', async () => {
      // given
      server.use(timedOutAuthCodeResponse);
  ...
  describe("서버로의 요청이 실패한 상태", () => {
    it('인증 번호 전송 요청이 실패하면 "이메일 전송에 실패했습니다. 잠시 후에 다시 시도해주세요." 메시지를 보여준다.', async () => {
      server.use(failedSendingAuthCodeResponse);
      const { renderResult, emailField, authCodeRequestButton } =
        renderAndGetFields();
      const { findByRole } = renderResult;
    ...
    it('인증 번호가 틀리면 "인증 번호가 올바르지 않습니다." 메시지를 보여준다.', async () => {
      server.use(
        ...[successSendingAuthCodeResponse, wrongAuthCodeForSignUpResponse]
      );
      const {
        renderResult,
        emailField,
