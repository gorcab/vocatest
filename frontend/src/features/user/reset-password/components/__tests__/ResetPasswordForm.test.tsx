...
import {
  ...
  unregisteredEmailForAuthCodeResponse,
  successSendingAuthCodeResponse,
  wrongAuthCodeForResetPasswordResponse,
...
describe("ResetPasswordForm", () => {
  ...
  describe("서버로의 요청이 실패한 상태", () => {
    ...
    it('회원가입 되지 않은 이메일로 인증 번호 요청을 시도하면 "가입되지 않은 이메일입니다." 메시지를 보여준다.', async () => {
      server.use(unregisteredEmailForAuthCodeResponse);
      const { renderResult, emailField, authCodeRequestButton } =
        renderAndGetFields();
      const { findByRole } = renderResult;
  ...
  it("비밀번호 재설정에 성공하면 handleSuccess prop을 호출한다.", async () => {
    server.use(
      ...[successSendingAuthCodeResponse, successResetPasswordResponse]
    );
    const {
      emailField,
      handleSuccess,
