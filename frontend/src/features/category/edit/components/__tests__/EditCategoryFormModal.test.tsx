...
import {
  existingCategoryNameForEditingCategoryResponse,
  successEditingCategoryResponse,
...
describe("EditCategoryFormModal", () => {
  ...
  it("카테고리 수정이 완료되면 `onClose` 함수를 호출한다.", async () => {
    server.use(successEditingCategoryResponse);
  ...
  it("카테고리 수정에 실패했을 경우 서버 측으로부터 받은 에러 메시지를 띄운다.", async () => {
    server.use(existingCategoryNameForEditingCategoryResponse);
